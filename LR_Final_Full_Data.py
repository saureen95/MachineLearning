""" This is the code for Logistic Regression on Full_Data.csv file.
    Accuracy achieved was 83.69% using LR model developed from scratch while
    Library implementation achieved 83.33% accuracy.
    """
...
def preprocessing(X):                                                                                       #Eliminating Punctuations and return numpy array
    Y=X.iloc[:,2:27]                                                                                        #Indexing to Headlines
    Y.replace(to_replace="[^a-zA-Z]", value=" ", regex=True, inplace=True)                                  
    Z=Y.values
    headlines=[]                                                                                            #Appending all headlines together in one list
    for i in range(len(Z)):
        headline=[]
        for j in range(len(Z[i])):
            if type(Z[i][j])==float:
                break
            Z[i][j]=Z[i][j].lower()                                                                         #lower case
            headline.append(Z[i][j])
        headlines.append(' '.join(headline))                                                                #Joining all headlines
    return headlines
...
news=pd.read_csv('Full_Data.csv', encoding = "ISO-8859-1")                                                  #Reading CSV File
news_date=news['Date']
...
ax.grid()
fig.savefig("LRPLOT_Full_Data.png")                                                                         #Figure saved as LRPLOT
